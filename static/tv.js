...
function createPlayers() {
  ...
  const guard = shell.querySelector(".player-guard");
  shell.insertBefore(second, guard || null);
  [first, second].forEach((el, index) => {
    el.style.position = "absolute";
    el.style.inset = "0";
    el.style.width = "100%";
    el.style.height = "100%";
    el.style.opacity = index === activeDeck ? "1" : "0";
  });
...
async function applyState(state, force = false) {
  ...
  if (song && (force || song.video_id !== currentVideo)) {
    if (transitioning && song.video_id === currentVideo) return;
